"""Integration: score the reverse-tabnabbing scanner through the evaluation harness."""
...
def test_reverse_tabnabbing_scanner_satisfies_detector_protocol() -> None:
    detector: Detector = ReverseTabnabbingScanner()  # structural conformance
    assert callable(detector.scan)


@pytest.mark.parametrize("corpus_id", ["reverse_tabnabbing_html", "reverse_tabnabbing_js"])
def test_reverse_tabnabbing_scored_by_harness(corpus_id: str) -> None:
    ...
    result = evaluate_corpus(
        ...
        detector_name="reverse-tabnabbing-scanner",
    )
