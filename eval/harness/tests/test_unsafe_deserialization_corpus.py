"""Integrity tests for the committed unsafe-deserialization curated corpora (CWE-502)."""
...
_CORPORA = {
    "unsafe_deserialization_py": "python",
    "unsafe_deserialization_js": "javascript",
}


@pytest.mark.parametrize("corpus_id", sorted(_CORPORA))
def test_unsafe_deserialization_corpus_loads_and_validates(corpus_id: str) -> None:
    ...
    assert {label.cwe for label in label_set.labels} == {"CWE-502"}
