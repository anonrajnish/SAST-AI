"""Integrity tests for the committed weak-cryptography curated corpora (CWE-327/328)."""
...
_CORPORA = {"weak_crypto_py": "python", "weak_crypto_js": "javascript"}


@pytest.mark.parametrize("corpus_id", sorted(_CORPORA))
def test_weak_crypto_corpus_loads_and_validates(corpus_id: str) -> None:
    ...
    assert {label.cwe for label in label_set.labels} == {"CWE-327", "CWE-328"}
